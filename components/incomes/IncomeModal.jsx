...
export default function IncomeModal({ open, onOpenChange, income, categories, onSuccess }) {
  const isEdit = !!income;
  const {
  ...
  const onSubmit = async (data) => {
    ...
    toast.success(isEdit ? "Kirim yangilandi!" : "Kirim qo'shildi!");
    onSuccess?.();
    onOpenChange(false);
  };
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle>{isEdit ? "Kirimni Tahrirlash" : "Yangi Kirim Qo'shish"}</DialogTitle>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <DialogFooter>
            ...
            <Button type="submit" disabled={isSubmitting} variant="income">
